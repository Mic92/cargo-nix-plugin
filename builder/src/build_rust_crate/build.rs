...
pub fn run(config: &mut BuildConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Set CARGO_PKG_* env vars — rustc needs these for env!() macros
    set_cargo_pkg_env(config);

    // Set rustc-env vars from build script.
    // Relative paths must be made absolute — include!() in source files
    // resolves relative to the source file, not the crate root.
    let cwd = std::env::current_dir()?;
    for (k, v) in &bso.envs {
        let val = if std::path::Path::new(v).is_relative() && std::path::Path::new(v).exists() {
    ...
    // Persist build script link search/lib flags to target/link and
    // target/link.final so downstream crates can link native libraries.
    // The bash setup_link_paths did this during the build phase.
    ...
    // Track lib --extern for bin linking
    ...
    if let Some(lib_path) = resolve_lib_path(config) {
        ...
        run_cmd(&mut flags.cmd(&crate_name, &lib_path, "target/lib", &crate_types, &extra, false), config.verbose)?;
        ...
        if config.build_tests {
            ...
            run_cmd(&mut flags.cmd(&crate_name, &lib_path, "target/lib", &crate_types, &extra, true), config.verbose)?;
        }
    }
    ...
    if config.build_tests && Path::new("tests").is_dir() {
        for entry in fs::read_dir("tests")?.flatten() {
            ...
            if p.extension().map(|e| e == "rs").unwrap_or(false) && (p.is_file() || p.is_symlink()) {
                ...
                build_bin(config, &flags, &lib_extern, &name, &p.join("main.rs").to_string_lossy(), true)?;
            }
        }
    }
...
/// Append build-script-discovered link search paths and link libs to
/// target/link and target/link.final, mirroring bash setup_link_paths.
/// For link.final, remap target/build paths to the installed lib output
/// so downstream crates reference valid store paths.
fn persist_bso_link_flags(bso: &BuildScriptOutputs, config: &BuildConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for search in &bso.link_search {
        ...
        // Remap build sandbox paths to installed output paths for downstream
    ...
    if !link_final_append.is_empty() {
        let mut f = fs::OpenOptions::new().append(true).open("target/link.final")?;
        f.write_all(link_final_append.as_bytes())?;
    }

    // Regenerate target/link_ (space-separated) for the linker flags
    let link_content = fs::read_to_string("target/link")?;
    fs::write("target/link_", link_content.lines().collect::<Vec<_>>().join(" "))?;
...
fn build_bin(
    ...
    run_cmd(&mut flags.cmd(&crate_name_, path, "target/bin", &["bin"], &extra, test), config.verbose)?;
...
fn resolve_bins(config: &BuildConfig) -> Vec<(String, String)> {
    ...
    if !config.crate_bin.is_empty() {
        for bin in &config.crate_bin {
            let name = bin.name.clone().unwrap_or_else(|| config.crate_name.clone());

            // Skip binaries whose requiredFeatures aren't all enabled
            if !bin.required_features.is_empty()
                && !bin.required_features.iter().all(|f| config.crate_features.contains(f))
            {
                eprintln!(
                    "Binary {name} not compiled due to not having all of the required features -- {:?} -- enabled.",
            ...
            if let Some(ref path) = bin.path {
                ...
                eprintln!("\x1b[0;1;31mERROR: failed to find file for binary target: {name}\x1b[0m");
                std::process::exit(1);
            }
        }
