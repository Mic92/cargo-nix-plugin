...
/// Top-level config deserialized from NIX_ATTRS_JSON_FILE.
...
pub struct BuildConfig {
    ...
    /// Store path of rustc, for --remap-path-prefix to break rust-src closure leak
    #[serde(default)]
    pub rustc_path: String,

    /// Flattened transitive dep lib-output store paths (for symlinking + scanning)
    #[serde(default)]
    ...
    /// Pre-computed metadata hash (computed in Nix for determinism)
    pub metadata: String,

    /// Per-dependency extern info computed at Nix eval time.
    /// Correctly handles multiple versions and renames.
    #[serde(default)]
...
impl BuildConfig {
    pub fn from_json_file(path: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // Export top-level ALL_CAPS string attrs as env vars.
        // With __structuredAttrs, crate overrides like
        //   CARGO_ENCODED_RUSTFLAGS = "";
        // end up in the JSON but not in the process environment.
        // This makes them work the same as without structuredAttrs.
        if let Ok(raw) = serde_json::from_str::<serde_json::Value>(&content) {
            if let Some(obj) = raw.as_object() {
                for (k, v) in obj {
                    if let Some(s) = v.as_str() {
                        // Only export keys that look like env vars (ALL_CAPS/underscores)
                        if !k.is_empty() && k.chars().all(|c| c.is_ascii_uppercase() || c == '_' || c.is_ascii_digit()) {
                            std::env::set_var(k, s);
                        }
                    }
                }
