...
pub fn run(config: &BuildConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Copy lib artifacts + create un-hashed symlinks
    if dir_has_files("target/lib") {
        ...
        for entry in fs::read_dir(&dst)?.flatten() {
            ...
            if (name.ends_with(".so") || name.ends_with(".dylib")) && name.contains(&format!("-{metadata}")) {
...
fn install_tests(config: &BuildConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for dir in ["target/bin", "target/lib"] {
        ...
        for entry in entries.flatten() {
            ...
            if !p.is_file() || !is_executable(&p) { continue; }
            if dir == "target/lib" && (name.ends_with(".rlib") || name.ends_with(lib_ext) || name.ends_with(".d")) {
                continue;
            }
...
fn copy_if_nonempty(src: &str, dst: &str) -> Result<(), Box<dyn std::error::Error>> {
    if Path::new(src).exists() && fs::read_to_string(src).map(|s| !s.is_empty()).unwrap_or(false) {
