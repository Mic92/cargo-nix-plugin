...
''
  ...
  # Offline mode: auto-detect edition/proc-macro from Cargo.toml and fix
  # --extern paths (.rlib -> .so for proc-macro deps). All logic is in
  # the read-crate-info binary; only present when cargoTomlInfo is set.
  if command -v read-crate-info >/dev/null 2>&1 && [[ -e Cargo.toml ]]; then
    eval "$(read-crate-info build Cargo.toml "$LIB_RUSTC_OPTS" "$BIN_RUSTC_OPTS" "${stdenv.buildPlatform.extensions.library}")"
    # Write marker so downstream crates know this is a proc-macro
    if [[ "''${CRATE_IS_PROC_MACRO:-}" = 1 ]]; then
      mkdir -p target/lib
      touch target/lib/proc-macro.marker
    fi
  fi
