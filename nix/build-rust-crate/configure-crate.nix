...
''
  ...
  # The lockfile resolver can't know fields that live only in a crate's
  # published Cargo.toml (not in Cargo.lock or the sparse registry index).
  # read-crate-info parses the manifest and emits shell assignments. Must
  # run before build.rs is compiled so edition/links/etc. reach it.
  CRATE_EDITION=""
  CRATE_BUILD=""
  CRATE_LINKS=""
  CRATE_LIB_PATH=""
  CRATE_LIB_NAME=""
  CRATE_LIB_TYPES=""
  CRATE_PKG_DESCRIPTION=""
  CRATE_PKG_REPOSITORY=""
  CRATE_PKG_LICENSE=""
  CRATE_PKG_HOMEPAGE=""
  CRATE_PKG_README=""
  CRATE_PKG_RUST_VERSION=""
  CRATE_PKG_AUTHORS=""
  if command -v read-crate-info >/dev/null 2>&1 && [[ -e Cargo.toml ]]; then
    eval "$(read-crate-info configure Cargo.toml)"
  fi

  # Resolver-provided values (interpolated at eval time) take precedence;
  # workspace members get real values from the resolver, external crates
  # get empty strings and fall through to Cargo.toml detection.
  BUILD="${build}"
  : "''${BUILD:=$CRATE_BUILD}"
  : "''${CARGO_MANIFEST_LINKS:=$CRATE_LINKS}"
  : "''${CARGO_PKG_DESCRIPTION:=$CRATE_PKG_DESCRIPTION}"
  : "''${CARGO_PKG_REPOSITORY:=$CRATE_PKG_REPOSITORY}"
  : "''${CARGO_PKG_LICENSE:=$CRATE_PKG_LICENSE}"
  : "''${CARGO_PKG_HOMEPAGE:=$CRATE_PKG_HOMEPAGE}"
  : "''${CARGO_PKG_README:=$CRATE_PKG_README}"
  : "''${CARGO_PKG_RUST_VERSION:=$CRATE_PKG_RUST_VERSION}"
  : "''${CARGO_PKG_AUTHORS:=$CRATE_PKG_AUTHORS}"
  export CARGO_MANIFEST_LINKS CARGO_PKG_DESCRIPTION CARGO_PKG_REPOSITORY \
         CARGO_PKG_LICENSE CARGO_PKG_HOMEPAGE CARGO_PKG_README \
         CARGO_PKG_RUST_VERSION CARGO_PKG_AUTHORS
  ...
  if [[ ! -z "$BUILD" ]] ; then
     ...
     # Fixup --extern paths for build deps: proc-macro .rlib -> .so and
     # [lib] name mismatches. The eval-time ${buildDeps} may reference
     # .rlib for proc-macro crates whose type isn't known until build time.
     BUILD_DEPS="${buildDeps}"
     if command -v read-crate-info >/dev/null 2>&1; then
       eval "$(read-crate-info fixup-deps "$BUILD_DEPS" "${stdenv.buildPlatform.extensions.library}")"
     fi
     # Only add CRATE_EDITION if rustcOpts doesn't already contain --edition
     # (the resolver may have set it via extraRustcOptsForBuildRs).
     EDITION_FLAG=""
     if [[ -n "$CRATE_EDITION" && "${rustcOpts}" != *--edition* ]]; then
       EDITION_FLAG="--edition $CRATE_EDITION"
     fi
     noisily rustc --crate-name build_script_build $BUILD --crate-type bin ${rustcOpts} $EDITION_FLAG \
