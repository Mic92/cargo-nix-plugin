...
#
# The Rust binary discovers --extern flags by scanning target/deps/ at
# build time, so Nix only needs to pass store paths — not per-dep metadata.
  ...
  (
    ...
    stdenv.mkDerivation (
      rec {
        ...
        # Per-dependency extern info for the Rust builder. Each entry
        # carries the extern name (with renames applied), the metadata
        # hash, and whether it's a proc-macro (needs .so not .rlib).
        # This replaces the old filesystem-based --extern discovery and
        # correctly handles multiple versions of the same crate.
        depExterns =
          let
            ...
            findRename = dep:
              if lib.hasAttr dep.crateName crateRenames then
                let
                  ...
                  findMatch = cs:
                    lib.findFirst (c: (!(c ? version) || c.version == dep.version or "")) { rename = normalizeName dep.libName; } cs;
                in
        ...
        # Flattened transitive dep store paths — used for symlinking
        # libraries to target/deps/ and collecting link flags.
        completeDeps =
          let
            deps = map lib.getLib dependencies_;
          in
          map toString (lib.unique (deps ++ lib.concatMap (dep: dep.completeDeps or [ ]) deps));
        # Exposed so tests can inspect build dep platform
        ...
        # Store path of rustc, used to remap rust-src paths and break closure leak
        rustcPath = toString rust;
        capLints = crate.capLints or capLints_;
        inherit release verbose;
        inherit hasCrateBin;
        inherit crateBin;

        # Rename info — only needed when extern name differs from lib target name
        inherit crateRenames;

        # Platform info for the Rust binary
        hostPlatform = {
