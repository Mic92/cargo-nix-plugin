...

{
  ...
  # When non-null, added to nativeBuildInputs of every crate derivation.
  cargoTomlInfo ? null,
}:
  ...
  mkRustcDepArgs =
    ...
    lib.concatMapStringsSep " " (
      ...
      let
        ...
        # Determine the library filename. For proc-macros, use the build
        # platform's shared library extension (.so/.dylib). When procMacro
        # status is unknown (offline mode), prefer .rlib but the build-time
        # read-crate-info binary will fix up the path if needed.
        filename =
  ...
  (
    ...
    stdenv.mkDerivation (
      rec {
        ...
        ++ nativeBuildInputs_
        ++ lib.optional (cargoTomlInfo != null) cargoTomlInfo;
        buildInputs =
