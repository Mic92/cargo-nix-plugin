...
let
  # Whether dependencies need unstable CLI (noprelude: for stdlib deps)
  ...
  (
    ...
    stdenv.mkDerivation (
      rec {
        ...
        # Each entry has the extern name (with renames applied), metadata
        # hash, and whether it's an rlib or proc-macro (.so).
        depExterns =
          let
        ...
        # Flattened transitive dep store paths for symlinking + link flags.
        completeDeps =
          let
        ...
        # Store path of rustc for --remap-path-prefix
        rustcPath = "${rust}";
